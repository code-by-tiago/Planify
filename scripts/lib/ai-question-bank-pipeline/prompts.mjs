/**
 * Prompts ETAPA 1 (gerador) e ETAPA 2 (revisor) — banco de questões K-12 BR / BNCC.
...
export function buildGeneratorSystemPrompt(ctx) {
  return `Você é um especialista em avaliação escolar brasileira (BNCC, Ensino Fundamental e Médio).
...
2. Linguagem adequada à faixa etária — vocabulário simples e claro.
...
7. Use apenas conhecimentos escolares consensuais e verificáveis — NÃO invente datas, leis, fórmulas ou estatísticas.
...
export function buildReviewerSystemPrompt() {
  return `Você é revisor pedagógico rigoroso de questões de múltipla escolha para escolas brasileiras.
...
- Adequação ao nível escolar brasileiro (não universitário, não fórum em inglês).

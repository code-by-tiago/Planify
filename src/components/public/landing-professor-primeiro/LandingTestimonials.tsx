  ...
  return (
    <section className="border-t border-slate-200/80 bg-white px-5 py-16 sm:px-8 sm:py-20">
      <div className="mx-auto max-w-7xl">
        ...
        <motion.blockquote
          className="relative mx-auto mt-12 max-w-3xl overflow-hidden rounded-3xl border border-cyan-200/80 bg-gradient-to-br from-cyan-50/80 via-white to-sky-50/50 p-8 shadow-md sm:p-10"
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {TESTIMONIALS.map((item, index) => (
            <motion.blockquote
              key={item.name}
              className="flex h-full flex-col rounded-2xl border border-slate-200/80 bg-slate-50/50 p-6 transition hover:border-cyan-200 hover:shadow-md"
              initial={reduce ? false : { opacity: 0, y: 16 }}
              whileInView={{ opacity: 1, y: 0 }}

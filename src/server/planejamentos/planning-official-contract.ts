...
/** Máximo de períodos por linha da matriz nos modelos oficiais. */
export const OFFICIAL_MAX_PERIODS_PER_ROW = 4;
...
export function splitPeriodsIntoExperienceChunks(totalPeriods: number): number[] {
  const total = Math.max(1, Math.floor(totalPeriods));
  const chunks: number[] = [];
  let remaining = total;

  while (remaining > 0) {
    if (remaining <= OFFICIAL_MAX_PERIODS_PER_ROW) {
      chunks.push(remaining);
      break;
    }

    chunks.push(OFFICIAL_PERIODS_PER_EXPERIENCE);
    remaining -= OFFICIAL_PERIODS_PER_EXPERIENCE;
  }

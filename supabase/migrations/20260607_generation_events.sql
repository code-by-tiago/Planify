-- Planify — Telemetria de gerações IA (sem PII).
-- Insert via service role (API server). Leitura admin via /api/admin/generation-stats.
...
-- Sem policies para anon/authenticated: apenas service_role insere/lê via backend.
